...
export class Sessions {
  ...
  private readonly KEYS = {
    PLAYER: {
      ...
      CURRENT_SESSION: (playerName: string) => ['player', playerName, 'currentSession'],
      ROLE: (playerName: string) => ['player', playerName, 'role'],
      PERMISSION_LEVEL: (playerName: string) => ['player', playerName, 'permissionLevel']
    },
    TICKETS: {
      PLAYER: (ticket: string) => ['tickets', 'player', ticket],
      PLAYER_NAME: (playerName: string) => ['tickets', 'playerName', playerName],
      ADMIN: () => ['tickets', 'admin'],
      PENDING_ADMIN_SOCKET: (socketId: string) => ['tickets', 'pending_admin_socket', socketId]
    },
    MAPPINGS: {
  ...
  private generateTicket(): string {
    ...
    return Array.from({ length: this.CONFIG.TICKET_LENGTH }, () =>
      chars.charAt(Math.floor(Math.random() * chars.length))
    ).join('');
  }

  private createLoginMessage(ticket: string, isAdmin: boolean = false): TellrawJSON {
    return container([
      isAdmin ?
  ...
  private async updatePlayerStats(kv: any, playerName: string, updates: Partial<PlayerStats>): Promise<void> {
    ...
    await kv.set(this.KEYS.PLAYER.STATS(playerName), { ...currentStats, ...updates });
  }
  ...
  @Event('player_joined')
  async handlePlayerJoined({ params, kv, log, tellraw }: ScriptContext): Promise<void> {
    ...
    try {
      ...
      const expiryTime = new Date(Date.now() + this.CONFIG.TICKET_EXPIRY).toISOString();
      ...
      await kv.atomic()
        ...
        .set(this.KEYS.TICKETS.PLAYER_NAME(playerName), { ticket: playerTicket, expiryTime })
        .commit();

      if (isAdmin && adminTicket) {
        await kv.set(this.KEYS.TICKETS.ADMIN(), { ticket: adminTicket, expiryTime });
      }
      ...
      await kv.atomic()
        .set(this.KEYS.PLAYER.SESSIONS(playerName), { [sessionId]: sessionData })
  ...
  async handlePlayerLeft({ params, kv, log }: ScriptContext): Promise<void> {
    ...
    try {
      ...
      const currentSessionResult = await kv.get(this.KEYS.PLAYER.CURRENT_SESSION(playerName));
      ...
      // Clean up tickets
      const ticketResult = await kv.get(this.KEYS.TICKETS.PLAYER_NAME(playerName));
      ...
      log(`Session closed for ${playerName} - Duration: ${Math.floor(duration / 60)}m ${Math.floor(duration % 60)}s`);
  ...
  @Permission('player')
  async loginCommand({ params, kv, tellraw, log }: ScriptContext): Promise<{ messages: any[] }> {
    const { sender } = params;
  ...
  @Permission('player')
  async sessionInfo({ params, kv, tellraw }: ScriptContext): Promise<{ messages: any[] }> {
    const { sender } = params;

    try {
      const currentSessionResult = await kv.get(this.KEYS.PLAYER.CURRENT_SESSION(sender));
      if (!currentSessionResult.value) {
      ...
      const duration = (Date.now() - new Date(sessionData.startTime).getTime()) / 1000;
      ...
      const infoMessage = container([
        ...
        text(sessionData.clientInfo?.version || 'unknown', { style: { color: 'white' } }),
        text('\n\n'),
        button('View History', {
          variant: 'ghost',
          onClick: {
  ...
  @Permission('player')
  async sessionHistory({ params, kv, tellraw }: ScriptContext): Promise<{ messages: any[] }> {
    const { sender } = params;
  ...
  async ticketAuth({ params, kv, auth, log }: ScriptContext): Promise<{ success: boolean; token?: any; error?: string }> {
    ...
    try {
      ...
      const isPendingAdmin = await kv.get(this.KEYS.TICKETS.PENDING_ADMIN_SOCKET(socket.id));
      ...
      const isOperator = isPendingAdmin.value || playerName === Deno.env.get('DENORITE_ADMIN_USER');
      ...
      if (isOperator) {
        ...
        if (!result.ok) {
          log(`Warning: Failed to update role and permissions for ${playerName}`);
        } else {
