...
export class FileSystem {
  ...
  async cd(context: ScriptContext): Promise<{ messages: any[], success?: boolean }> {
  ...
  async rm({ params, tellraw, log, kv }: ScriptContext): Promise<{ messages: any[], success?: boolean }> {
    ...
    try {
      ...
      const confirmMsg = container([
        ...
        text("Are you sure you want to delete this ", { style: { color: "gray" } }),
  ...
  async rmConfirm({ params, tellraw, log, kv }: ScriptContext): Promise<{ messages: any[], success?: boolean }> {
    const { sender, args } = params;
    const currentDir = await this.getCurrentDir(kv, sender);
    const targetPath = this.resolvePath(currentDir, args.path);
